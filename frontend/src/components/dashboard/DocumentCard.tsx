...
const TYPE_BADGE: Record<DocumentItem['type'], { label: string; color: string }> = {
  blank: { label: 'Canvas', color: 'bg-[#E7E5E4] text-[#44403C]' },
  template: { label: 'Template', color: 'bg-[#FEF3C7] text-[#92400E]' },
  markdown: { label: 'Markdown', color: 'bg-[#DBEAFE] text-[#1E40AF]' },
  docx: { label: 'Word', color: 'bg-[#EDE9FE] text-[#5B21B6]' },
  pdf: { label: 'PDF', color: 'bg-[#FEE2E2] text-[#991B1B]' },
};
...
const DocIcon: React.FC<{ type: DocumentItem['type'] }> = ({ type }) => {
  ...
  return <FileText className="w-5 h-5 text-[#78716C]" />;
};
...
export const DocumentCard: React.FC<DocumentCardProps> = ({
  ...
  if (viewMode === 'list') {
    return (
      <div className="flex items-center gap-4 px-4 py-3 hover:bg-[#FAF8F5] rounded-xl transition-colors group border border-transparent hover:border-[#E7E5E4]">
        {/* Doc Icon */}
        <div className="w-9 h-9 rounded-lg bg-[#F4F0EA] border border-[#E7E5E4] flex items-center justify-center shrink-0">
          <DocIcon type={doc.type} />
        </div>

        {/* Title + meta */}
        <div className="flex-1 min-w-0 text-left cursor-pointer" onClick={() => onOpen(doc)}>
          <p className="text-sm font-semibold text-[#1C1917] truncate group-hover:text-[#D97706] transition-colors">
            {doc.title}
          </p>
          <div className="flex items-center gap-2 text-[11px] text-[#78716C] mt-0.5">
            <span>{doc.ownerName}</span>
            {doc.myRole && doc.myRole !== 'OWNER' && (
              <span className={`px-1.5 py-0.5 rounded text-[10px] font-semibold ${doc.myRole === 'VIEWER' ? 'bg-[#DCFCE7] text-[#14532D]' : 'bg-[#EDE9FE] text-[#5B21B6]'}`}>
        ...
        {!isTrashed && (
          <button
            ...
            className={`p-1.5 rounded-lg transition-colors ${doc.isStarred ? 'text-[#F59E0B]' : 'text-[#D6D3D1] hover:text-[#F59E0B]'}`}
        ...
        <div ref={menuRef} className="relative">
          <button
            ...
            className="p-1.5 rounded-lg text-[#D6D3D1] hover:text-[#1C1917] hover:bg-[#E7E5E4] transition-colors opacity-0 group-hover:opacity-100"
          >
  ...
  // Grid Mode
  return (
    <div className="group bg-[#FFFFFF] border border-[#E7E5E4] rounded-2xl overflow-hidden shadow-editorial hover:shadow-editorial-hover transition-all duration-200 hover:border-[#D6D3D1] flex flex-col">
      {/* Card Header / Preview Area */}
      <div
        className="h-28 bg-[#FAF8F5] border-b border-[#E7E5E4] p-4 flex flex-col justify-between cursor-pointer"
        onClick={() => onOpen(doc)}
      >
        <div className="flex items-start justify-between">
          <div className="w-10 h-10 rounded-xl bg-[#FFFFFF] border border-[#E7E5E4] shadow-xs flex items-center justify-center">
            <DocIcon type={doc.type} />
          </div>
      ...
      <div className="p-4 flex-1 flex flex-col justify-between gap-3">
        <div className="text-left cursor-pointer" onClick={() => onOpen(doc)}>
          <h4 className="text-sm font-semibold text-[#1C1917] line-clamp-2 group-hover:text-[#D97706] transition-colors leading-snug">
            {doc.title}
          </h4>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex -space-x-1.5 items-center">
            {doc.collaborators.slice(0, 3).map(c => (
              <div
                key={c.id}
                className="w-5 h-5 rounded-full border-2 border-white text-[8px] font-bold flex items-center justify-center text-white"
                style={{ backgroundColor: c.color }}
                title={c.name}
              >
                {c.name[0]}
          ...
          <div ref={menuRef} className="relative">
            <button
              onClick={() => setMenuOpen(!menuOpen)}
              className="p-1.5 rounded-lg text-[#D6D3D1] hover:text-[#1C1917] hover:bg-[#E7E5E4] transition-colors"
            >
              <MoreHorizontal className="w-4 h-4" />
            </button>
            {menuOpen && (
...
const ContextMenu: React.FC<{
  ...
  <div className="absolute right-0 bottom-8 z-20 bg-[#FFFFFF] border border-[#E7E5E4] rounded-xl shadow-lg p-1 min-w-[150px] animate-in fade-in zoom-in-95 duration-150">
    {!isTrashed ? (
      <>
